...
/**------------------------------------------------------------------------
 **                           Engagement
 *?  Renders a list of the entries for the given resource (Engagement indicators)
 *------------------------------------------------------------------------**/
...
function Engagement() {
   ...
   */
  const fetchEngagementIndicators = async () => {
    const options = {method: 'GET', url: `${api_endpoint}/engagement`};

    axios.request(options).then(function (response) {
      setEngagementIndicators(response.data);
  ...
  */
  useEffect(() => {
    fetchEngagementIndicators();
  }, [])

  return (
    <div className='py-4 px-2'>
      <div className='flex flex-row items-center gap-3'>
        <h1 className='font-extrabold text-dark-blue'>Indicadores de Engagement</h1>
        <button>
          <FontAwesomeIcon icon={faCirclePlus} color = '#EF8354' size = "2xl" onClick={() => {
            navigate("/indicator/create", { state: { resource: 'engagement' }})
          }}/>
        </button>
      </div>
      ...
      <div className="relative overflow-x-auto shadow-md sm:rounded-md">
        <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            <tbody>
              {engagementIndicators && engagementIndicators.map((indicator) => (
                  <tr className="bg-white border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-300">
                    ...
                    <td className="px-6 py-4 text-right">
                        <button
                           onClick={() => {
                            navigate("/indicator/edit", { state: { indicator: indicator, type: 'engagement'  } });
                           }}
                        >
