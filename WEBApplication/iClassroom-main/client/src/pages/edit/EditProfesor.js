...
/**------------------------------------------------------------------------
 **                           EditProfessor
 *?  Renders form to edit an existing professor 
 *------------------------------------------------------------------------**/
...
function EditProfesor() {
   ...
   */
  useEffect(() => {
    setName(state.profesor.name);
    setEmail(state.profesor.email);
    setLastName(state.profesor.last_name)
  }, [])
      ...
      buttons: [
        {
          ...
          onClick: () => {
            ...
            axios.request(options).then(function (response) {
              ...
              navigate("/admin", { state: { resource: 'teachers' } });
    ...
    <div>
      <div className = 'edit-container'>
        <div className='flex flex-row items-center gap-4'>
          <h1 className='mb-2'>Editando Profesor</h1>
        ...
        <div className="text-center justify-content-center">
          ...
          <div className='flex flex-row gap-2'>
            <button className='btn-admin-forms' onClick={()=>{updateTeacher()}}>
              Actualizar registro
            </button>
            <button className='btn-admin-forms-back' onClick={()=>{navigate("/admin", { state: { resource: 'teachers' } });}}>
              Regresar
            </button>
          </div>
        </div>
